...
> 5. **The browser client IS the TS SDK.** `@openagentmesh/sdk` is transport-agnostic: it picks `wsconnect` for `ws://`/`wss://` URLs (the Node TCP transport is a dynamic import the browser build never pulls in) and already has a `configUrl` connect option that fetches `{nats_ws_url}` — the exact `/config.json` bootstrap this ADR describes. The UI depends on the SDK (workspace link to `sdk-ts/` until the npm publish lands) instead of reimplementing call/stream/catalog-watch. Related: `nats.ws` (named throughout this ADR) is deprecated upstream; the ecosystem moved to `@nats-io/nats-core` + `@nats-io/jetstream` + `@nats-io/kv`, which the SDK already uses. Read "nats.ws" below as "the SDK's websocket transport".
