...
- **Wildfire incident-response demo** (ADR-0054, `python -m demos.wildfire`): a full multi-fleet scenario running on the mesh with no orchestrator — a fire simulation, high-altitude UAVs that spot heat and write deduplicated detections to KV, drones that claim each detection by compare-and-swap, and heli/firefighter/medevac fleets that act on what the drones survey, plus LLM peers that brief incidents, turn an operator's plain-English request into a validated dispatch command, and narrate the incident. One command boots embedded NATS and the whole fleet; a Svelte scenario dashboard renders the world and the ADR-0056 admin UI renders the mesh beside it. The cascade runs with no LLM key — only briefing, natural-language dispatch and narration need one. New cookbook recipe: "Wildfire Incident Response".

- **Persona-team experiment demo** (`python -m openagentmesh.demos.persona_team --stub`): research machinery comparing a standing team of peer personas (shared KV blackboard, randomized round-robin Delphi rounds, no orchestrator) against a hierarchical orchestrator-and-workers baseline on the same design question — built entirely from shipped primitives (KV CAS, `mesh.call`, usage attribution, observability events). The runner emits a JSON report per run with wall time, per-agent token usage, and mesh message counts. Measured comparisons require an OpenRouter key; the stub mode dry-runs the machinery offline with clearly-labeled synthetic numbers.

- **LLM usage attribution** (ADR-0023): LLM-backed agents can report what each request cost with `report_usage(Usage(input_tokens=..., output_tokens=..., model=...))` from anywhere inside the handler — no output-schema changes, and multiple reports per request accumulate. The mesh stamps the merged result on the `X-Mesh-Usage` reply header (readable from any language) and publishes a `usage_reported` observability event per reporting request, so a per-agent cost monitor is just `mesh.observe.logs()` plus a counter — see the new "Tracking LLM Usage" cookbook recipe. Reporting is opt-in; agents that don't report emit nothing.

- **Admin UI** (ADR-0056): see the mesh from the browser with `oam mesh up` + `oam ui`. Three screens: an agent registry (capability badges, live/offline status dots driven by real instance tracking and death notices), an invocation sandbox (a form generated from each agent's input schema — Call renders the typed reply, Stream renders chunks live with a Stop button, and errors render with their taxonomy code, including `not_available` from lifecycle-gated agents), and a live event feed (wiretap any subject pattern with pause/resume and clear). There is no HTTP API layer: the browser is a first-class mesh client over the new NATS WebSocket listener that the dev mesh (`oam mesh up`, `AgentMesh.local()`) opens on mesh port + 1. Release wheels bundle the compiled UI — no Node.js needed at runtime.

- **Agent lifecycle gates** (ADR-0055): declare when an agent should be online with `@mesh.agent(spec, active_when=mesh.kv_condition("incident.mode", predicate))` — the agent subscribes while the condition holds and drains + unsubscribes when it stops holding, with no controller process. Gates compose with `sources` (a gated agent's sources deliver nothing while it is offline), the catalog entry stays visible either way, and every replica converges on the same state by watching the same signal. `mesh.subject_condition(...)` gates on plain NATS messages instead of KV. Calling a gated-offline agent raises the new `NotAvailable` (code `not_available`) — distinct from `NotFound`, so callers know to retry when the condition changes. Gate transitions publish `agent_activated`/`agent_deactivated` observability events.
...
- TypeScript SDK observability primitives: `mesh.tap(pattern)` wiretaps any subject pattern and yields every message with its subject (JSON decoded, raw text fallback; error envelopes are yielded with an `isError` flag instead of thrown), and `mesh.instancesWatch()` streams live snapshots of the `mesh-instances` bucket so clients can track which agents have live hosts. Both power the admin UI's event feed and registry status dots.
