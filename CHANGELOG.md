...
- **Admin UI groundwork** (ADR-0056, in progress): the dev mesh (`oam mesh up`, `AgentMesh.local()`) now also opens a NATS WebSocket listener on mesh port + 1, so browsers can join the mesh directly. New `oam ui` command serves the admin UI and hands the browser its connection config; the UI screens themselves ship in a later release (until then `oam ui` explains how to run the frontend dev server from source).
